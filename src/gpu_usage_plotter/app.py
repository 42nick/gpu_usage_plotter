...
def update_graph_live(n, gpu_usage_data):
    ...
    df = pd.DataFrame.from_dict(gpu_usage_data["df"])
    df = pd.concat([df, get_gpu_usage()], ignore_index=True)

    fig = px.line(df, x="timestamp", y=["load"], color="name")
    fig2 = px.line(df, x="timestamp", y=["memoryUtil"], color="name")

    gpu_usage_data["df"] = df.to_dict()
...
def func(n_clicks, gpu_usage_data):
